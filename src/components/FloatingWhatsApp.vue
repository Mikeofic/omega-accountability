<template>
  <div class="fixed bottom-5 right-5 z-50 flex flex-col items-end gap-2">
    <span class="px-3 py-1 rounded-full text-xs font-medium bg-surface/90 border border-black/10 text-text" :class="withinBusinessHours ? 'ring-1 ring-green-500/30' : 'opacity-80'">
    ...
    <a :href="whatsUrl" target="_blank" rel="noopener" :aria-label="ariaLabel" :title="title"
       class="bg-green-500 hover:bg-green-600 text-white p-4 rounded-full shadow-lg transition-transform transform hover:scale-110 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-400 focus:ring-offset-white">
